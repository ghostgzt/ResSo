...
<tr>
<td>
<a Title="狼视影院 - 天狼星の破晓" target="_blank" href="http://mv.godwolfs.asia/index.html" ><img src="./tpl/em.jpg" border="0" /></a>
</td>
<td>
<a Title="云点播 - 天狼星の破晓" target="_blank" href="http://mv.godwolfs.asia/yun.php" ><img  src="./tpl/zm.gif" border="0" /></a>
</td>
<td>
...
<td><input class="keyword" type="text" id="kw" name="q" value="<?php if(!isset($_REQUEST['z'])){$_REQUEST['z']='';}if(!isset($_REQUEST['s'])){$_REQUEST['s']='';} if($_REQUEST['z']&&$_REQUEST['s']!='1'){echo $_REQUEST["z"];}else{echo '请输入要搜索的关键字';} ?>" x-webkit-speech="" x-webkit-grammar="builtin:translate" onwebkitspeechchange="document.getElementById('kw').value=document.getElementById('kw').value.replace('请输入要搜索的关键字','');" /></td>
...
<input class="keyword" type="text" id="query" name="query" value="<?php if(!isset($_REQUEST['z'])){$_REQUEST['z']='';}if(!isset($_REQUEST['s'])){$_REQUEST['s']='';} if($_REQUEST['z']&&$_REQUEST['s']=='1'){echo $_REQUEST["z"];}else{echo '请输入要搜索的关键字';} ?>" x-webkit-speech="" x-webkit-grammar="builtin:translate" onwebkitspeechchange="document.getElementById('query').value=document.getElementById('query').value.replace('请输入要搜索的关键字','');" />
...
<form action="./bt.php" method="get" target="iix">
...
<input class="keyword" type="text" id="title" name="title" value="<?php if(!isset($_REQUEST['z'])){$_REQUEST['z']='';}if(!isset($_REQUEST['s'])){$_REQUEST['s']='';} if($_REQUEST['z']&&$_REQUEST['s']=='1'){echo $_REQUEST["z"];}else{echo '请输入要搜索的关键字';} ?>" x-webkit-speech="" x-webkit-grammar="builtin:translate" onwebkitspeechchange="document.getElementById('title').value=document.getElementById('title').value.replace('请输入要搜索的关键字','');" />
<input type="submit" id="search" value="BT搜索" onclick="var ix=0;if(encodeURIComponent(document.getElementById('title').value).length<1){alert('至少1个字符！');return false;ix=0;}else{ix=1;}if(document.getElementById('title').value=='请输入要搜索的关键字'){return false;ix=0;}else{ix=1}if(ix==1){document.title='搜索结果: '+document.getElementById('title').value+' - BT搜索引擎 - 天狼星の破晓'}"/>
</form>
</div>
