...
$html=str_replace('<a href="/search','<a href="'.'http://'.$_SERVER["HTTP_HOST"].'/'.$mfile.'?search',$html);
$html=str_replace('<a class="next" href="/','<a class="next" href="'.'http://'.$_SERVER["HTTP_HOST"].'/'.$mfile.'?',$html);
preg_match_all('/page\_content">(.*?)<div class="aside">/is', $html, $match);
            	//echo $html;
           echo '<br/><br/><div id="page_content">'.$match	[1][0].'<div class="aside"><iframe width="120" scrolling="no" height="600" frameborder="no" framespacing="0" src="'.'http://'.$_SERVER["HTTP_HOST"].'/tpl/gad.htm'.'"></iframe></div>';
